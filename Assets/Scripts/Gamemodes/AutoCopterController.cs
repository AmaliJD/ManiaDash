...
public class AutoCopterController : PlayerController
{
    ...
    public void Rotate()
    {
        ...
        else if (player_body.velocity.x >= 0)
        {
            ...
            //transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(newAngle), .2f);
            float newAngle = rev * -player_body.velocity.x;
            player_body.rotation = Mathf.LerpAngle(player_body.rotation, newAngle, .5f);
        }
        else
        {
            int rev = reversed ? -1 : 1;
    ...
    public override void Jump()
    {
        ...
        player_body.AddForce(new Vector2(0, goingUp ? (mini ? 60f : 45f) : (mini ? -60f : -45f)));
