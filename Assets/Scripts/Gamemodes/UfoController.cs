...
public class UfoController : PlayerController
{
    ...
    void Update()
    {
        if (able)
        {
            ...
            if ((grounded || player_body.velocity.y > 0.1) && !grounded_particles.isPlaying) { grounded_particles.Play(); }
            else if(grounded_particles.isPlaying) { grounded_particles.Stop(); }
    ...
    public void Rotate()
    {
        ...
        bool floored = Physics2D.BoxCast(player_body.transform.position, new Vector2(.95f, .1f), 0f, !reversed ? Vector2.down : Vector2.up, .51f, groundLayer)
                        && (Physics2D.IsTouchingLayers(player_collider, groundLayer) || Physics2D.IsTouchingLayers(ufo_collider, groundLayer));

        if ((Mathf.Abs(player_body.velocity.x) < 0.2f) || floored)
        {
            player_body.freezeRotation = true;
            player_body.rotation = Mathf.Lerp(player_body.rotation, player_body.rotation < 180 ? 0 : 360, .5f);
        }
        else if (player_body.velocity.x > 0)
        {
            if (player_body.velocity.y >= 0)
            {
                float newAngle = 360 - (Mathf.Abs(player_body.velocity.y) * (norm / maxSpeed));
                player_body.rotation = newAngle;
            }
            else
            {
                float newAngle = Mathf.Abs(player_body.velocity.y) * (rev / maxSpeed);
                player_body.rotation = newAngle;
            }
        }
        else if (player_body.velocity.x < 0)
        {
            if (player_body.velocity.y >= 0)
            {
                float newAngle = Mathf.Abs(player_body.velocity.y) * (norm / maxSpeed);
                player_body.rotation = newAngle;
            }
            else
            {
                float newAngle = 360 - (Mathf.Abs(player_body.velocity.y) * (rev / maxSpeed));
                player_body.rotation = newAngle;
            }
        }
    }
    ...
    public override void Jump()
    {
        ...
        else if (red && jump)
        {
            ...
            maxSpeed = Mathf.Abs(jumpForce) * 1.75f;
            player_body.velocity = new Vector2(player_body.velocity.x, jumpForce * 1.75f);
