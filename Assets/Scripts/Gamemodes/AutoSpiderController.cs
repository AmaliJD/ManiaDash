...
public class AutoSpiderController : PlayerController
{
    ...
    void Update()
    {
        ...
        {
            // CHECK IF DEAD
            dead = check_death && (Physics2D.IsTouchingLayers(spider_collider, deathLayer) || Mathf.Abs(player_body.velocity.x) <= 0.01);
    ...
    public override void Move()
    {
        ...
        if (grounded && Mathf.Abs(player_body.velocity.y) <= 0.01 && (!red_p && !yellow_p && !blue_p && !pink_p && !gravN && !grav))
        {
            ...
            //Spider_Anim.GetComponent<Animator>().Play("run", -1, 0f);
