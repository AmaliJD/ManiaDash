...
public class AspectRatioEnforcer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(Screen.fullScreen == false && w/h != 16/9 && (lastWidth == w && lastHeight == h))// && !(Input.mousePosition.x <= 1 || Input.mousePosition.y <= 1 || Input.mousePosition.x >= Screen.width - 2 || Input.mousePosition.y >= Screen.height - 2))
        {
            Screen.SetResolution((int)w, (int)(w * 9/16), false);
        }
    ...
    void Update()
    {
        ...
        if (lastWidth != width) // if the user is changing the width
        {
            ...
            Screen.SetResolution(width, Mathf.RoundToInt(heightAccordingToWidth), false);
        }
        else if (lastHeight != height) // if the user is changing the height
        {
            // update the width
            float widthAccordingToHeight = height / 9.0f * 16.0f;
            Screen.SetResolution(Mathf.RoundToInt(widthAccordingToHeight), height, false);
        }
