...
{
    public bool matchP1Opacity;
    private float opactity = 1;
    public Slider sliderOpacityOverride;
    ...
    void getSaveData()
    {
        ...
        if (File.Exists(path))
        {
            ...
            GlobalData savedata = formatter.Deserialize(stream) as GlobalData;
            opactity = savedata.p1_opacity;
            stream.Close();
        }
        else
        {
    ...
    protected override void AssignColor(Color color, float hue, float sat, float val, float alpha)
    {
        ...
        /*if (matchP1Opacity)
        {
            //alpha = sliderOpacityOverride != null ? (sliderOpacityOverride.value / 4f) - 1 : opactity - 1;
            if(!setColorAlpha1)
            {
                color.a = 1;
        ...
        if (matchP1Opacity)
        {
            a = sliderOpacityOverride == null ? opactity : sliderOpacityOverride.value / 4f;
        }
        ...
        if (alwaysbright && !brightOverride)
        {
            if(v <= .5f)
            {
                s = s * Mathf.Sqrt(v * 2);
